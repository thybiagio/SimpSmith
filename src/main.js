...
import { CANVAS_WIDTH, CANVAS_HEIGHT, MAP_OFFSET } from './config.js'
...
import { rectangularCollision, checkForCharacterCollision, checkForOreCollision } from './systems/collision.js'


const { boundaries, characters, ores } = buildWorld()
...
const background = new Sprite({
  position: { x: MAP_OFFSET.x, y: MAP_OFFSET.y },
  image
})
...
const movables = [background, ...boundaries, foreground, ...characters, ...ores]
const renderables = [background, ...boundaries, ...characters, ...ores, player, foreground]
...
checkForOreCollision({ ores, player, oreOffset: { x: 0, y: 3 } })
    ...
    checkForOreCollision({ ores, player, oreOffset: { x: 3, y: 0 } })
    for (let i = 0; i < boundaries.length; i++) {
      const boundary = boundaries[i]
      if (
        rectangularCollision({
